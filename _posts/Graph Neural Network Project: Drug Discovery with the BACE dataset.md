...
There are several different types of graph neural network tasks [1] and in this project we are interested in graph classification. This problem involved predicting whether a molecule, which can be represented as a graph, is a potential drug. As discussed before, molecules are described well by graphs as the nodes can represent atoms and the edges represent the bonds between them. 
